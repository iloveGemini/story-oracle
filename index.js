    ...
    win.innerHTML = `
        ...
        <div id="so-fixwarn">
            <div id="so-fixwarn-card">
                <div id="so-fixwarn-head"><i class="fa-solid fa-circle-info"></i> ✨ 自动校正：一键去 AI 味</div>
...
// fixA_pieceAsked（之后不再弹，判定行仍显示待确认）。确认（confirmPieceBare）后按聊天记住 bare，
// 之后每条新回复照常分段校正。不确认：后续回复静默跳过——不确定性是可见的问题，绝不是无声的猜测。
...
async function fixPieceCall(ctx, s, a, directive, piece, signal, onDelta) {
    ...
    if (s.mode === 'direct') {
        ...
        if (onDelta && s.stream) {
            let acc = '';
            finalText = await streamDirect(normalizeUrl(s.endpoint), s.apiKey, body, signal, (delta) => { acc += delta; onDelta(acc); });
        } else {
            finalText = await callDirect(normalizeUrl(s.endpoint), s.apiKey, body, signal);
        }
    } else {
        const override = s.sendTemperature ? { temperature: s.temperature } : {};
        if (onDelta && s.stream) {
    ...
    if (fixNoOp(pieceProse, parsed.fixed)) return { status: 'clean' };
...
async function runAutoFix(ctx, s, targetId) {
    ...
    if (ENABLE_FIX_PIECEWISE && fixPieceDecision) {
        ...
        if (pd.action === 'pending') return;                    // 判定行显示待确认；不逐条打扰（无声但可见）
...
function buildBackendModelsPayload(endpoint, apiKey) {
    const customUrl = modelsUrl(endpoint).replace(/\/models$/, '');
    const headers = apiKey ? { Authorization: 'Bearer ' + apiKey } : {};
    return {
        chat_completion_source: 'custom',
        custom_url: customUrl,
...
async function fetchModelsViaBackend(endpoint, apiKey, signal) {
    const ctx = getCtx();
...
async function onFetchModels() {
    ...
    try {
        ...
        if (s.directViaBackend) {
            ...
            data = await fetchModelsViaBackend(s.endpoint, s.apiKey, signal);
        } else {
